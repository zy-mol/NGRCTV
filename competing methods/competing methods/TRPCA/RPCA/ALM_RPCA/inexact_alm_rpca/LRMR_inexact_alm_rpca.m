function [A_hat,E_hat,iter] = LRMR_inexact_alm_rpca(D, r,k,lambda, tol, maxIter)
...
%     E_v=zeros(1,m*n);
%     % 需要构造指定稀疏个数的稀疏矩阵
%     E_vect=reshape(E_hat,1,m*n);%列为主序
%     E_tmp=sort(abs(E_vect));
%     ind=find(E_vect>E_tmp(end-k) | E_vect<-E_tmp(end-k));
%     E_v(ind)=E_vect(ind);
%     E_hat=reshape(E_v,m,n);
...
% E_vect=abs(reshape(E_hat,1,m*n));%列为主序
% E_tmp=sort(E_vect);
% ind=find(E_vect>E_tmp(end-k));
% E_v(ind)=E_vect(ind);
% E_hat=reshape(E_v,m,n);
converged
